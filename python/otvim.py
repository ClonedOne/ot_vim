...
class OTVim:
    ...
    if len(b1) < len(b2):
        ...
        elif cursor_y == 0 and len(b2[cursor_x]) > 0:
        ## TODO: HANDLE ENTIRE STRING
            return (cursor_x, 0, b2[cursor_x], OperationType.INSERT)
    ...
    #  a line was removed
        ...
        elif cursor_y == 0 and len(b2[cursor_x]) == len(b1[cursor_x]):
        ## TODO: HANDLE ENTIRE STRING
            return (cursor_x + 1, 0, b2[cursor_x + 1], OperationType.DELETE)
    ...
    #  the operation is on the same line
        ...
        s1 = set([(cursor_x, pos, b1[cursor_x][pos]) for pos in range(len(b1[cursor_x]))])
        s2 = set([(cursor_x, pos, b2[cursor_x][pos]) for pos in range(len(b2[cursor_x]))])

        #  insert or change operation
        if len(b1[cursor_x]) <= len(b2[cursor_x]):
            return s2 - s1
